 ...
 * extension.
 */
class CRM_Twingle_ExtensionUtil {
  const SHORT_NAME = "twingle";
  const LONG_NAME = "de.systopia.twingle";
  const CLASS_PREFIX = "CRM_Twingle";
 ...
 */
function _twingle_civix_civicrm_xmlMenu(&$files) {
  foreach (_twingle_civix_glob(__DIR__ . '/xml/Menu/*.xml') as $file) {
    $files[] = $file;
  }
}
 ...
 */
function _twingle_civix_civicrm_enable() {
  _twingle_civix_civicrm_config();
  if ($upgrader = _twingle_civix_upgrader()) {
 ...
 * @return mixed
 */
function _twingle_civix_civicrm_disable() {
  _twingle_civix_civicrm_config();
  if ($upgrader = _twingle_civix_upgrader()) {
 ...
 */
function _twingle_civix_insert_navigation_menu(&$menu, $path, $item) {
  ...
  if (empty($path)) {
    ...
    foreach ($menu as $key => &$entry) {
      if ($entry['attributes']['name'] == $first) {
        ...
        $found = _twingle_civix_insert_navigation_menu($entry['child'], implode('/', $path), $item, $key);
      }
    }
    return $found;
  }
 ...
 */
function _twingle_civix_navigationMenu(&$nodes) {
  if (!is_callable(array('CRM_Core_BAO_Navigation', 'fixNavigationMenu'))) {
    _twingle_civix_fixNavigationMenu($nodes);
  }
}
 ...
 * missing them.
 */
function _twingle_civix_fixNavigationMenu(&$nodes) {
  $maxNavID = 1;
  ...
  _twingle_civix_fixNavigationMenuItems($nodes, $maxNavID, NULL);
}

function _twingle_civix_fixNavigationMenuItems(&$nodes, &$maxNavID, $parentID) {
  ...
  foreach ($origKeys as $origKey) {
    ...
    if (isset($nodes[$origKey]['child']) && is_array($nodes[$origKey]['child'])) {
      _twingle_civix_fixNavigationMenuItems($nodes[$origKey]['child'], $maxNavID, $nodes[$origKey]['attributes']['navID']);
    }
  }
}
